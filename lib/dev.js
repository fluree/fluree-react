...
function ReactConnect(connSettings) {
  ...
  workerInvoke({
    ...
    cb: function cb(response) {
      ...
      var conn = response.conn;
      var data = { status: (response.status === 200 ? "loading" : "connection error")};
      if (conn && connStatus[conn.id].cb)
      {
        connStatus[conn.id].cb.forEach(compId =>
          {
  ...
  function (_React$Component2) {
    ...
    _createClass(FlurQL, [{
      ...
      value: function UNSAFE_componentWillMount() {
        ...
        if (!connStatus[this.conn.id].ready) {
          console.log("Adding component to connection list: ", this.id)
          if (!connStatus[this.conn.id].cb) {connStatus[this.conn.id].cb = []};
          connStatus[this.conn.id].cb.push(this.id); // update state when connection available
        }
        else if (this.query && this.isValidQuery) {
          console.log("registering query");
          registerQuery(this.conn, this.id, this.query, this.opts);
        }
        else {
          console.log("huh? ", connStatus[this.conn.id].ready);
        }
      }
    }, {
      ...
      value: function render() {
        ...
        var data = {
          ...
          forceUpdate: function () {
            if (connStatus[this.conn.id].ready && this.query && this.isValidQuery) registerQuery(this.conn, this.id, this.query, this.opts, true);  
          }.bind(this),
