...
function workerMessageHandler(e) {
  ...
  switch (msg.event) {
    ...
    case "connReset":
      var crResponse = msg.data || {};
      var crStatusCode = crResponse.status;
...
function ReactConnect(config) {
  ...
  var conn = {
    ...
    login: function login(username, password, expire, options, _cb) {
      return workerInvoke({
        conn: safeConfig.id,
        action: "login",
      ...
      return workerInvoke({
        ...
        cb: function cb(response) {
          ...
          if (_cb2 && typeof _cb2 === 'function') {
            if (response.status === 200 && options && options.rememberMe) localStorage.setItem(localStorageKey, response.result); // username, token

            _cb2(response.result);
          } // execute pending callbacks on connection object
    ...
    transact: function transact(txn, options, _cb3) {
      var _txn = txn && typeof txn === 'string' ? txn : JSON.stringify(txn);
